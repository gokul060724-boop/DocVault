...
// ==========================================
// SECURITY CREDENTIAL UPDATE ROUTES
// ==========================================
...
app.post("/api/verify-current-password", requireLogin, async (req, res) => {
    try {
        ...
        if (!correct) {
            return res.status(401).json({
                success: false,
                message: "Incorrect account password. Please re-enter it."
            });
        }
...
app.post("/api/verify-current-security-pin", requireLogin, async (req, res) => {
    try {
        ...
        if (!correct) {
            return res.status(401).json({
                success: false,
                message: "Incorrect Security PIN. Please re-enter it."
            });
        }
        ...
        return res.status(500).json({
            success: false,
            message: "Unable to verify current Security PIN."
        });
    }
});
...
app.post("/api/change-security-pin", requireLogin, async (req, res) => {
    try {
        ...
        if (!/^\d{6}$/.test(currentPin)) {
            return res.status(400).json({
                success: false,
                message: "Current Security PIN must contain exactly 6 digits."
            });
        }
        ...
        if (!/^\d{6}$/.test(newPin)) {
            return res.status(400).json({
                success: false,
                message: "New Security PIN must contain exactly 6 digits."
            });
        }
        ...
        if (newPin !== confirmPin) {
            return res.status(400).json({
                success: false,
                message: "New Security PIN and confirmation do not match."
            });
        }
        ...
        console.error("CHANGE SECURITY PIN ERROR:", error);
